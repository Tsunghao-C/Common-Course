...
static int	ft_print_wid1(long *nbr, char **wid_pre, char *flags, int *prt_len)
{
	...
	if (*nbr < 0 && ft_have_zero(flags) == 1 && !wid_pre[1])
	{
		wid -= 1;
		count += write(STDOUT_FILENO, "-", 1);
...
static int	ft_print_pre1(long *nbr, char *base, int *prt_len, char *flags)
{
	int	count;
	int	dgt_cnt;
